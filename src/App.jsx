...
export default function App() {
  ...
  const [showActionSheet, setShowActionSheet] = useState(false); // FIX: declarat corect
  const [toast, setToast]         = useState(null);
  const [tabDir, setTabDir]       = useState(1);

  const touchStartX = useRef(null);
  const TAB_ORDER = ['dashboard', 'azi', 'progres', 'antrenament', 'profil', 'setari'];
  ...
  return (
    <div style={{ height: '100dvh', background: th.bg, color: th.text, fontFamily: "'Inter','SF Pro',-apple-system,sans-serif", display: 'flex', flexDirection: 'column', overflow: 'hidden', maxWidth: '480px', margin: '0 auto', position: 'relative' }}>
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}
        ...
        {/* DASHBOARD — ecranul principal nou */}
        {tab === 'dashboard' && (
          <div key="dashboard" className={`tab-slide-${tabDir > 0 ? 'right' : 'left'}`}
            style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
            <DashboardTab
              onNavigate={(dest) => {
                if (dest === 'meals') setShowFoodSearch(true);
                else if (dest === 'gym') setGymMode(true);
                else if (dest === 'stats') switchTab('progres');
                else if (dest === 'coach') switchTab('azi');
              }}
            />
          </div>
        )}
        ...
        {tab === 'azi' && (
          <div key="azi" className={`tab-slide-${tabDir > 0 ? 'right' : 'left'}`} style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <AziTab
        ...
        {tab === 'alimente' && (
          <div key="alimente" className={`tab-slide-${tabDir > 0 ? 'right' : 'left'}`} style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <AlimenteTab th={th}
        ...
        {tab === 'antrenament' && (
          <div key="antrenament" className={`tab-slide-${tabDir > 0 ? 'right' : 'left'}`} style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        {tab === 'progres' && (
          <div key="progres" className={`tab-slide-${tabDir > 0 ? 'right' : 'left'}`} style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        {tab === 'profil' && (
          <div key="profil" className={`tab-slide-${tabDir > 0 ? 'right' : 'left'}`} style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        {tab === 'setari' && (
          <div key="setari" className={`tab-slide-${tabDir > 0 ? 'right' : 'left'}`} style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* ── BOTTOM TAB BAR ── */}
      <div style={{
        background: darkMode ? 'rgba(7,10,18,0.98)' : 'rgba(255,255,255,0.98)',
        borderTop: `1px solid ${th.border}`,
        display: 'flex', alignItems: 'flex-end', justifyContent: 'space-around',
        paddingBottom: 'max(14px,env(safe-area-inset-bottom))', paddingTop: '8px',
        flexShrink: 0, backdropFilter: 'blur(20px)', zIndex: 10,
        position: 'relative', minHeight: '76px',
      }}>

        {/* Dashboard */}
        <button onClick={() => switchTab('dashboard')} className="btn-tap"
          ...
          <div style={{ width: '34px', height: '34px', borderRadius: '12px', background: tab === 'dashboard' ? (currentDay?.bg || 'linear-gradient(135deg,#f97316,#ef4444)') : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', transition: 'all 0.2s' }}>🏠</div>
          <span style={{ fontSize: '11px', fontWeight: tab === 'dashboard' ? 800 : 500, color: tab === 'dashboard' ? th.accent : th.text3, transition: 'all 0.2s' }}>Home</span>
        </button>
